...
import it.agid.spid.saml.core.utils.StringUtil;
import it.agid.spid.saml.spid.utils.SoggettoAggregatoreConvention;
import it.agid.spid.saml.spid.utils.SpidAggregatorEntityTypeEnum;
/**
 * Singleton per la costruzione del metadata di un ServiceProvider
 *
 */
public class SpSoggettoAggregatoreMetadataBuilder extends AbstractMetadataBuilder {
	private static final Logger logger = LoggerFactory.getLogger(SpSoggettoAggregatoreMetadataBuilder.class.getName());
	...
	private static SpSoggettoAggregatoreMetadataBuilder _INSTANCE;
	/**
	 * Costruttore privato
	 */
	private SpSoggettoAggregatoreMetadataBuilder() {
	}
	/**
	 * Recupero l'istanza dell'oggetto
	 * @return -l'istanza 
	 */
	public static synchronized SpSoggettoAggregatoreMetadataBuilder getInstance() {
		if( _INSTANCE == null ) {
			_INSTANCE = new SpSoggettoAggregatoreMetadataBuilder();
		}
		return _INSTANCE;
	}
	/**
	 ...
	 * @param informazioniSoggettoAggregatore Riferimenti da utilizzare per costruire gli oggetti {@link ContactPerson} contenente le informazioni del soggetto aggregatore
	 * @param informazioniSoggettoAggregato Riferimenti da utilizzare per costruire gli oggetti {@link ContactPerson} contenente le informazioni del soggetto aggregato
	...
	public EntityDescriptor buildSpMetadata(String entityId, 
			...
			SamlOtherContactPerson informazioniSoggettoAggregatore,
			SamlOtherContactPerson informazioniSoggettoAggregato,
		...
		ed.getContactPersons().add(buildContactPerson(informazioniSoggettoAggregatore, true, entePrivato));
		ed.getContactPersons().add(buildContactPerson(informazioniSoggettoAggregato, false, entePrivato));
		if( !informazioniSoggettoAggregatore.isSpPubblico() ) {
	...
	private ContactPerson buildContactPerson(SamlOtherContactPerson spidAggregator, boolean cpSoggettoAggregatore, boolean entePrivato) {
		ContactPerson cp = OpenSAMLUtils.buildSAMLObject(ContactPerson.class);
		cp.setType(ContactPersonTypeEnumeration.OTHER);
		
		if( cpSoggettoAggregatore ) {

			cp.getUnknownAttributes().put(new QName(SPID_NS_URI, "entityType", SPID_NS_PREFIX), SpidAggregatorEntityTypeEnum.AGGREGATOR.getEntityType());
		}else {
			cp.getUnknownAttributes().put(new QName(SPID_NS_URI, "entityType", SPID_NS_PREFIX), SpidAggregatorEntityTypeEnum.AGGREGATED.getEntityType());
		}
		if( !StringUtil.isEmptyString(spidAggregator.getCompanyName()) ) {

			Company company = OpenSAMLUtils.buildSAMLObject(Company.class);
			company.setName(spidAggregator.getCompanyName());
			cp.setCompany(company);
		}else if( cpSoggettoAggregatore ) {
			throw new IllegalArgumentException("Passato un company name del soggetto aggregatore non corretto");
		}
		...
		if(cpSoggettoAggregatore) {
			...
			ipaCode.setTextContent(spidAggregator.getIpaCode());
			extensions.getUnknownXMLObjects().add(ipaCode);
			XSAny vatNumber = builder.buildObject(SPID_NS_URI,"VATNumber",SPID_NS_PREFIX);
			vatNumber.setTextContent(spidAggregator.getVatNumber());
			extensions.getUnknownXMLObjects().add(vatNumber);
			XSAny codFisc = builder.buildObject(SPID_NS_URI,"FiscalCode",SPID_NS_PREFIX);
			codFisc.setTextContent(spidAggregator.getFiscalCode());
			extensions.getUnknownXMLObjects().add(codFisc);
			String tagName = null;
			if( entePrivato ) {
				tagName = SoggettoAggregatoreConvention.AGGREGATORE_FULL_SERVIZI_PRIVATI.getMetadataTagName();
			}else {
				tagName = SoggettoAggregatoreConvention.AGGREGATORE_FULL_SERVIZI_PUBBLICI.getMetadataTagName();
			}
			extensions.getUnknownXMLObjects().add(builder.buildObject(SPID_NS_URI, tagName, SPID_NS_PREFIX));
			...
			extensions.getUnknownXMLObjects().add(ipaCode);
			if( entePrivato ) {
				extensions.getUnknownXMLObjects().add(builder.buildObject(SPID_NS_URI,"Private",SPID_NS_PREFIX));
			}else {
		...
		if( !StringUtil.isEmptyString(spidAggregator.getEmailAddress()) ) {
			...
			cp.getEmailAddresses().add(mailAggregatore);
		}else if( cpSoggettoAggregatore ) {
			throw new IllegalArgumentException("Passato un indirizzo mail del soggetto aggregatore non corretto");
		}
		if( !StringUtil.isEmptyString(spidAggregator.getTelephoneNumber()) ) {
			TelephoneNumber tn = OpenSAMLUtils.buildSAMLObject(TelephoneNumber.class);
			tn.setNumber(spidAggregator.getTelephoneNumber());
			cp.getTelephoneNumbers().add(tn);
		}
		if( StringUtil.isEmptyString(spidAggregator.getIpaCode()) ) {
			throw new IllegalArgumentException("Passato un IPA code aggregatore nullo");
		}
